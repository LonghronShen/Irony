...
namespace Irony.Interpreter
{
    ...
    public class ScriptInterpreter
    {
        ...
        public Task WorkerThread { get; private set; }
#else
        ...
        public void EvaluateAsync()
        {
            ...
            WorkerThread = TaskEx.Run(() => AsyncThreadStart(null), _cancellationTokenSource.Token);
#else
        ...
        public async Task EvaluateTaskAsync(string script)
        {
            Script = script;
            await EvaluateTaskAsync();
        }

        public async Task EvaluateTaskAsync(ParseTree parsedScript)
        {
            ParsedScript = parsedScript;
            await EvaluateTaskAsync();
        }

        public async Task EvaluateTaskAsync()
        {
            CheckNotBusy();
            Status = _internalStatus = InterpreterStatus.Evaluating;
            _cancellationTokenSource = new CancellationTokenSource();
            await TaskEx.Run(() => AsyncThreadStart(null), _cancellationTokenSource.Token);
        }
        ...
        public void Abort()
        {
            try
            {
                ...
                WorkerThread.Wait(50, _cancellationTokenSource.Token);
#else
                WorkerThread.Abort();
